...
int main(int argc, char* argv[]) {
	if (argc != 3)
		throw_error(ERR_CONSOLE_ARGUMENTS, nullptr);

	init_vectors();
	file_read_lines(argv[2], &v_str, &v_times);
	...
	for (unsigned long i = 0; i < strings->size; i++) {
		...
		if (startswith(cur_line.str, L"M107")) {
			...
			if (get_param(cur_line.str, L"S") <= current_fan) {
				...
				GcodeLine* bef_line;
				if (target_index == 0)
					bef_line = (GcodeLine*)cvector_get(lines, 0);
				else
					bef_line = (GcodeLine*)cvector_get(lines, target_index-1);
				GcodeLine* line1 = NULL;
				GcodeLine* line2 = NULL;
				if (fabs(target_time - bef_line->time) < 1e-6 || target_index == 0 ||
					!gcode_split_line((GcodeLine*)cvector_get(lines, target_index),
								      bef_line, target_time-bef_line->time, &line1, &line2)) {
...
int fan_stack_proc(CVector* lines, CVector* fan_lines, const GcodeLine* line,
	...
	while (fan_lines->size && *((unsigned long*)cvector_get(fan_lines, fan_lines->size-1)) > target_index) {
		...
		unsigned long fan_line_index = *(unsigned long*)cvector_get(fan_lines, fan_lines->size-1);
		GcodeLine* fan_line = (GcodeLine*)cvector_get(lines, fan_line_index);
		assert(fan_line != NULL);

		if (startswith(fan_line->str, L"M107")) {
			fan_line->str = (wchar_t*)realloc(fan_line->str, sizeof(wchar_t));
			...
			*fan_line->str = '\0';
			...
			if (fan_line->pos.fan_prev >= current_fan) {
				...
				fan_line->str = (wchar_t*)realloc(fan_line->str, (wcslen(line->str) + 1) * sizeof(wchar_t));
				if (fan_line->str == NULL)
					throw_error(ERR_ALLOC, nullptr);
				...
				fan_line->str = (wchar_t*)realloc(fan_line->str, 1 * sizeof(wchar_t));
