...
int gcode_split_line(GcodeLine* cur_line, GcodeLine* bef_line, double delta, GcodeLine** out1, GcodeLine** out2) {
	...
	if (cur_line->pos.XYZabs) {
		if (is_param(cur_line->str, L"X")) {
			...
			swprintf(buf, sizeof(buf), L" X%.6f", pos1.X);
			wcscat(s1, buf);
			swprintf(buf, sizeof(buf), L" X%.6f", pos2.X);
			wcscat(s2, buf);
		}
		if (is_param(cur_line->str, L"Y")) {
			pos1.Y = bef_line->pos.Y + (cur_line->pos.Y - bef_line->pos.Y) * delta / full_time;
			swprintf(buf, sizeof(buf), L" Y%.6f", pos1.Y);
			wcscat(s1, buf);
			swprintf(buf, sizeof(buf), L" Y%.6f", pos2.Y);
			wcscat(s2, buf);
		}
		if (is_param(cur_line->str, L"Z")) {
			pos1.Z = bef_line->pos.Z + (cur_line->pos.Z - bef_line->pos.Z) * delta / full_time;
			swprintf(buf, sizeof(buf), L" Z%.6f", pos1.Z);
			wcscat(s1, buf);
			swprintf(buf, sizeof(buf), L" Z%.6f", pos2.Z);
			wcscat(s2, buf);
		}
	} else {
		double dbuf = 0.0;
		if (get_param_ptr(cur_line->str, L"X", &dbuf)) {
			...
			swprintf(buf, sizeof(buf), L" X%.6f", dbuf * delta / full_time);
			wcscat(s1, buf);
			swprintf(buf, sizeof(buf), L" X%.6f", dbuf * (full_time - delta) / full_time);
			wcscat(s2, buf);
		}
		if (get_param_ptr(cur_line->str, L"Y", &dbuf)) {
			pos1.Y += dbuf * delta / full_time;
			swprintf(buf, sizeof(buf), L" Y%.6f", dbuf * delta / full_time);
			wcscat(s1, buf);
			swprintf(buf, sizeof(buf), L" Y%.6f", dbuf * (full_time - delta) / full_time);
			wcscat(s2, buf);
		}
		if (get_param_ptr(cur_line->str, L"Z", &dbuf)) {
			pos1.Z += dbuf * delta / full_time;
			swprintf(buf, sizeof(buf), L" Z%.6f", dbuf * delta / full_time);
			wcscat(s1, buf);
			swprintf(buf, sizeof(buf), L" Z%.6f", dbuf * (full_time - delta) / full_time);
			wcscat(s2, buf);
		}
	}
	if (cur_line->pos.Eabs) {
		if (is_param(cur_line->str, L"E")) {
			...
			swprintf(buf, sizeof(buf), L" E%.6f", pos1.E);
			wcscat(s1, buf);
			swprintf(buf, sizeof(buf), L" E%.6f", pos2.E);
			wcscat(s2, buf);
		}
	} else {
		double dbuf = 0.0;
		if (get_param_ptr(cur_line->str, L"E", &dbuf)) {
			...
			swprintf(buf, sizeof(buf), L" E%.6f", dbuf * delta / full_time);
			wcscat(s1, buf);
			swprintf(buf, sizeof(buf), L" E%.6f", dbuf * (full_time - delta) / full_time);
			wcscat(s2, buf);
		}
	}
	...
	*out1 	     = (GcodeLine*)calloc(1, sizeof(GcodeLine));
	(*out1)->str = (wchar_t*)calloc(wcslen(s1), sizeof(wchar_t));
	...
	*out2 		 = (GcodeLine*)calloc(1, sizeof(GcodeLine));
	(*out2)->str = (wchar_t*)calloc(wcslen(s2), sizeof(wchar_t));
